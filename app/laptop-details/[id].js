...
const laptopdata = () => {
  ...
  useEffect(() => {
    if (!id) {
      ...
      axios
        .get(`https://project-x-five-smoky.vercel.app/api/laptop?id=${id}`)
        .then((response) => {
    ...
    <ScrollView>
      <View>
        ...
        >
          ...
          <Text style={{ paddingTop: 10, marginBottom: 20, fontSize: 16 }}>
            {product?.short}
          </Text>
          ...
          <Text style={{ color: "gray", fontSize: 15 }}>{product?.rom}</Text>
        </View>
...
const UserModel = (props) => {
  ...
  const sizes = ["white", "grey", "black", "silver"];
